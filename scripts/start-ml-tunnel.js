...
async function updateRenderEnv(ngrokUrl) {
  ...
  if (getRes.status !== 200) {
    throw new Error(`Gagal ambil env vars Render: ${getRes.status} ${JSON.stringify(getRes.body)}`);
  }
  ...
  if (putRes.status !== 200) {
    throw new Error(`Gagal update env vars: ${putRes.status} ${JSON.stringify(putRes.body)}`);
  }
