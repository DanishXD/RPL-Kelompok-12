 ...
 * Kirim data real-time ke backend setiap 1 detik.
 *
 * Usage:
...
const CONFIG = {
  ...
  DEVICE_TOKEN:
    "377dd62b1563420e45eea956e53683b5d50accada5d48a050e35c0515ce0dd0f",
  INTERVAL: 1000,
};
// ─────────────────────────────────────────────────────────────────────────────
...
function generateSensorData() {
  ...
  const temperature = +(
    26 +
    3 * Math.sin(iteration * 0.3) +
    (Math.random() - 0.5)
  ).toFixed(1);
  const phLevel = +(
    7.1 +
    0.4 * Math.sin(iteration * 0.15) +
    (Math.random() - 0.5) * 0.2
  ).toFixed(2);
  const feedLevel = +Math.max(
    45,
    95 - (iteration % 100) + (Math.random() - 0.5) * 2,
  ).toFixed(1);
  const hour = Math.floor(iteration / 12) % 24;
  const isDay = hour >= 6 && hour <= 18;
  const lightLevel = Math.round(
    isDay
...
async function login() {
  ...
  if (!data.data?.accessToken)
    throw new Error(`Login gagal: ${JSON.stringify(data)}`);
...
async function main() {
  ...
  console.log(
    `🔑 Mode     : ${CONFIG.DEVICE_TOKEN ? "ingest (deviceToken)" : "test-alert (JWT)"}`,
  );
