...
TIME_LABELS = {
    0: {
        "label": "Pagi",
        "time": "06:00–08:00",
        "description": "Waktu pagi optimal untuk pemberian pakan",
    },
    1: {
        "label": "Siang",
        "time": "12:00–13:00",
        "description": "Waktu siang untuk pemberian pakan",
    },
    2: {
        "label": "Sore",
        "time": "17:00–18:00",
        "description": "Waktu sore optimal untuk pemberian pakan",
    },
    3: {
        "label": "Tidak Direkomendasikan",
        "time": None,
        "description": "Kondisi sensor tidak mendukung pemberian pakan saat ini",
    },
}
    ...
    return jsonify(
        {
            ...
            "data": {
                "time_slot": {
                    "best_model": meta_time.get("best_model"),
                    "accuracy": meta_time.get("accuracy"),
                    "cv_accuracy": meta_time.get("cv_accuracy_mean"),
                    "f1_weighted": meta_time.get("f1_weighted"),
                    "cv_f1": meta_time.get("cv_f1_mean"),
                    "features": meta_time.get("feature_importance", {}),
                },
                "amount_gram": {
                    "best_model": meta_amount.get("best_model"),
                    "accuracy": meta_amount.get("accuracy"),
                    "cv_accuracy": meta_amount.get("cv_accuracy_mean"),
                    "f1_weighted": meta_amount.get("f1_weighted"),
                    "cv_f1": meta_amount.get("cv_f1_mean"),
...
def predict():
    if model_time is None or model_amount is None:
        return jsonify(
            {
                "success": False,
                "error": "Model belum diload. Jalankan train_model.py dulu.",
            }
        ), 503
    ...
    if missing:
        return jsonify(
            {"success": False, "error": f"Field wajib tidak ada: {missing}"}
        ), 400
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": f"Tipe data tidak valid: {str(e)}"}
        ), 400
    ...
    if not (0 <= feed_level <= 100):
        return jsonify(
            {"success": False, "error": "feed_level harus antara 0–100"}
        ), 400
