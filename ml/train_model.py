...
def run_comparison(X, y, target_name, class_labels):
    ...
    b1 = ax.bar(x - 0.2, test_accs, 0.35, label='Test Accuracy',         color=colors, alpha=0.90)
    b2 = ax.bar(x + 0.2, cv_means,  0.35, label='CV Accuracy (5-fold)',  color=colors, alpha=0.55,
                yerr=cv_stds, capsize=5)

    ax.set_ylabel('Accuracy (%)', fontsize=12)
    ax.set_title(f'Perbandingan Akurasi — {target_name}', fontsize=13, fontweight='bold')
    ax.set_xticks(x)
    ...
    ax.annotate(
        '★ Terbaik',
        xy        = (best_idx, max(test_accs[best_idx], cv_means[best_idx]) + 5),
        ha        = 'center', fontsize=11,
        color     = MODEL_COLORS[best_name], fontweight='bold',
    )
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    b1 = ax.bar(x - 0.2, f1_scores,   0.35, label='Test F1 Score (weighted)',        color=colors, alpha=0.90)
    b2 = ax.bar(x + 0.2, cv_f1_means, 0.35, label='CV F1 Score (5-fold, weighted)',  color=colors, alpha=0.55,
                yerr=cv_f1_stds, capsize=5)

    ax.set_ylabel('F1 Score (%)', fontsize=12)
    ax.set_title(f'Perbandingan F1 Score — {target_name}', fontsize=13, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels(names, fontsize=11)
    ax.set_ylim(0, 110)
    ax.legend(fontsize=10)
    ...
    ax.annotate(
        '★ Terbaik',
        xy        = (best_f1_idx, max(f1_scores[best_f1_idx], cv_f1_means[best_f1_idx]) + 5),
        ha        = 'center', fontsize=11,
        color     = MODEL_COLORS[names[best_f1_idx]], fontweight='bold',
    )
    ...
    metadata = {
        ...
        'all_results'      : results,
        'feature_importance': (
            {f: round(float(v), 4) for f, v in zip(FEATURES, best_model.feature_importances_)}
