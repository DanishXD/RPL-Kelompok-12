...
export class AuthService {
  ...
  async login(input: LoginInput) {
    ...
    if (!user) { await bcrypt.compare(input.password, '$2b$12$invalidhashfortimingattack00000'); throw { statusCode: 401, message: 'Email atau password salah' }; }
    const isValid = await bcrypt.compare(input.password, user.passwordHash);
    if (!isValid) throw { statusCode: 401, message: 'Email atau password salah' };
  ...
  async refreshAccessToken(token: string) {
    ...
    const [stored] = await db.select().from(refreshTokens)
      .where(and(eq(refreshTokens.token, token), eq(refreshTokens.isRevoked, false), gt(refreshTokens.expiresAt, new Date()))).limit(1);
    if (!stored) throw { statusCode: 401, message: 'Refresh token tidak valid atau expired' };
  ...
  async logout(token: string) {
    await this.fastify.db.update(refreshTokens).set({ isRevoked: true }).where(eq(refreshTokens.token, token));
  ...
  async getProfile(userId: string) {
    ...
    const userDevices = await db.select({ id: devices.id, name: devices.name, status: devices.status, location: devices.location, lastSeenAt: devices.lastSeenAt })
  ...
  private async generateTokens(user: { id: string; email: string; role: 'admin' | 'user' }) {
    const db = this.fastify.db;
    const payload: JwtPayload = { sub: user.id, email: user.email, role: user.role, tokenVersion: '1' };
    const accessToken = this.fastify.jwt.sign(payload, { expiresIn: process.env.JWT_ACCESS_EXPIRES ?? '15m' });
    const refreshTokenValue = crypto.randomBytes(40).toString('hex');
    const expiresAt = new Date(); expiresAt.setDate(expiresAt.getDate() + 30);
    await db.insert(refreshTokens).values({ token: refreshTokenValue, userId: user.id, expiresAt });
