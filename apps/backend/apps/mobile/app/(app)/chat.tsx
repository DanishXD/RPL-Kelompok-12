...
export default function ChatScreen() {
  ...
  const send = async (text?: string) => {
    ...
    try {
      const { data } = await api.post('/chat/message', {
        message:   msg,
        deviceId:  deviceId,   // otomatis dari SecureStore
        sessionId: sessionId,
      });
      ...
      setMessages(prev => [...prev, {
        ...
        role: 'assistant',
        time: fmt(new Date()),
        content: err?.response?.data?.error?.includes('GROQ_API_KEY')
          ? '⚠️ GROQ_API_KEY belum diisi di .env backend.\nDaftar gratis di console.groq.com'
          : '❌ Gagal terhubung ke AI. Pastikan backend berjalan dan GROQ_API_KEY sudah diisi di .env',
      }]);
      setApiReady(false);
    } finally {
      setLoading(false);
  ...
  const clearChat = () => {
    ...
    setMessages([{
      ...
      role:    'assistant',
      time:    fmt(new Date()),
      content: 'Percakapan baru dimulai! Ada yang ingin ditanyakan tentang kolam kamu?',
    }]);
  };
      ...
      <ScreenHeader
        ...
        rightElement={
          <View style={styles.headerRight}>
            <View style={[styles.aiBadge, { backgroundColor: apiReady ? Colors.successBg : Colors.dangerBg }]}>
              <View style={[styles.aiDot, { backgroundColor: apiReady ? Colors.success : Colors.danger }]} />
              <Text style={[styles.aiText, { color: apiReady ? Colors.success : Colors.danger }]}>
          ...
          {messages.map(m => (
            <View key={m.id} style={[styles.msgRow, m.role === 'user' && styles.msgRowUser]}>
              ...
              <View style={[styles.bubble, m.role === 'user' ? styles.bubbleUser : styles.bubbleBot]}>
                <Text style={[styles.bubbleTxt, m.role === 'user' && styles.bubbleTxtUser]}>
                  {m.content}
                </Text>
...
const styles = StyleSheet.create({
  ...
  aiBadge:      { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20 },
  aiDot:        { width: 7, height: 7, borderRadius: 4 },
  aiText:       { fontSize: 11, fontWeight: '700' },
  clearBtn:     { backgroundColor: 'rgba(255,255,255,0.2)', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12 },
