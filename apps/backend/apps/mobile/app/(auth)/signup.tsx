...
export default function SignupScreen() {
  ...
  const handleRegister = async () => {
    ...
    try { await register(name.trim(), email.trim().toLowerCase(), pass); }
    catch (err: any) {
    ...
    <KeyboardAvoidingView style={{ flex:1 }} behavior={Platform.OS==='ios'?'padding':'height'}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          <TouchableOpacity style={styles.backBtn} onPress={() => router.back()}><Ionicons name="arrow-back" size={22} color={Colors.white} /></TouchableOpacity>
        ...
        <View style={styles.card}>
          ...
          <FormInput label="Nama Lengkap" placeholder="Budi Suryanto" value={name} onChangeText={(t)=>{setName(t);clear('name');}} error={errors.name} returnKeyType="next" autoCapitalize="words" />
          <FormInput label="Email" placeholder="budi@example.com" keyboardType="email-address" value={email} onChangeText={(t)=>{setEmail(t);clear('email');}} error={errors.email} returnKeyType="next" />
          <FormInput label="Password" placeholder="Min. 8 karakter" isPassword value={pass} onChangeText={(t)=>{setPass(t);clear('password');}} error={errors.password} returnKeyType="next" />
          {pass.length > 0 && (
            <View style={styles.strengthRow}>
              <View style={styles.strengthBars}>{[1,2,3].map(l=><View key={l} style={[styles.strengthBar,{backgroundColor:strength.level>=l?strength.color:Colors.border}]} />)}</View>
              <Text style={[styles.strengthLabel,{color:strength.color}]}>{strength.label}</Text>
            </View>
          )}
          <FormInput label="Konfirmasi Password" placeholder="Ulangi password" isPassword value={confirm} onChangeText={(t)=>{setConfirm(t);clear('confirm');}} error={errors.confirm} returnKeyType="done" onSubmitEditing={handleRegister} />
          <View style={styles.reqs}>
            {[{text:'Minimal 8 karakter',met:pass.length>=8},{text:'Ada huruf besar',met:/[A-Z]/.test(pass)},{text:'Ada angka',met:/[0-9]/.test(pass)}].map(r=>(
              <View key={r.text} style={styles.reqRow}>
                <Ionicons name={r.met?'checkmark-circle':'ellipse-outline'} size={14} color={r.met?Colors.success:Colors.textMuted} />
                <Text style={[styles.reqText,{color:r.met?Colors.success:Colors.textMuted}]}>{r.text}</Text>
              </View>
            ))}
          </View>
          ...
          <View style={styles.loginRow}>
            ...
            <TouchableOpacity onPress={() => router.back()}><Text style={styles.loginLink}>Masuk di sini</Text></TouchableOpacity>
          </View>
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  header:        { backgroundColor:Colors.primary, paddingTop:60, paddingBottom:52, alignItems:'center', gap:8, borderBottomLeftRadius:32, borderBottomRightRadius:32 },
  backBtn:       { position:'absolute', top:60, left:20, width:40, height:40, borderRadius:20, backgroundColor:'rgba(255,255,255,0.15)', alignItems:'center', justifyContent:'center' },
  logoWrap:      { width:72, height:72, borderRadius:36, backgroundColor:'rgba(255,255,255,0.15)', alignItems:'center', justifyContent:'center', marginBottom:4 },
