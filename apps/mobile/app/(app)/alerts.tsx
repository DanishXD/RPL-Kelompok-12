...
export default function AlertsScreen() {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScreenHeader title="Notifikasi & Alert" subtitle={`${alerts.length} alert aktif saat ini`} showBack
        rightElement={alerts.length > 0 ? <View style={styles.activeBadge}><Text style={styles.activeBadgeText}>{alerts.length} Aktif</Text></View> : undefined}
      />
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        {alerts.map((alert, i) => {
          const isCritical = alert.alerts.some(a => (a.field === 'feedLevel' && a.value < 10) || (a.field === 'temperature' && (a.value > 34 || a.value < 22)));
          return (
            <View key={i} style={[styles.alertCard, isCritical ? styles.alertDanger : styles.alertWarning]}>
              <View style={styles.alertHeader}>
                ...
                <Text style={styles.alertTitle}>{alert.alerts.map(a => a.field === 'temperature' ? (a.status === 'high' ? 'Suhu Terlalu Tinggi' : 'Suhu Terlalu Rendah') : a.field === 'phLevel' ? (a.status === 'high' ? 'pH Terlalu Tinggi' : 'pH Terlalu Rendah') : 'Level Pakan Rendah').join(', ')}</Text>
                <View style={[styles.levelBadge, { backgroundColor: isCritical?Colors.dangerBg:Colors.warningBg, borderColor: isCritical?Colors.danger:Colors.warning }]}>
                  <Text style={[styles.levelText, { color: isCritical?Colors.danger:Colors.warning }]}>{isCritical ? 'Kritis' : 'Peringatan'}</Text>
                </View>
              </View>
              <Text style={styles.alertTime}>{alert.timestamp ? new Date(alert.timestamp).toLocaleString('id-ID') : '—'}</Text>
              <Text style={styles.alertDesc}>{alert.alerts.map(a => `${a.field === 'temperature' ? 'Suhu' : a.field === 'phLevel' ? 'pH' : 'Pakan'}: ${a.value} (${a.status === 'high' ? 'melebihi' : 'di bawah'} batas)`).join('\n')}</Text>
              <TouchableOpacity onPress={() => dismissAlert(i)}>
                <Text style={[styles.alertAction, { color: isCritical ? Colors.danger : Colors.primary }]}>→ Tandai selesai</Text>
              </TouchableOpacity>
            </View>
          );
        })}
          ...
          {HISTORY.map((h,i,arr) => (
            <React.Fragment key={h.id}>
              <View style={styles.histRow}>
                ...
                <View style={styles.resolvedBadge}><Text style={styles.resolvedText}>Selesai</Text></View>
              </View>
              {i<arr.length-1 && <View style={styles.divider} />}
            </React.Fragment>
          ))}
...
const styles = StyleSheet.create({
  ...
  saveBtn:       { backgroundColor:Colors.primary, borderRadius:14, paddingVertical:16, alignItems:'center', elevation:4 },
