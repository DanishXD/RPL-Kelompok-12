...
export default function DashboardScreen() {
  ...
  const [deviceName, setDeviceName] = useState<string>('Memuat device...');

  // Ambil device pertama milik user secara otomatis
  useEffect(() => {
    api.get('/devices')
      .then((res) => {
        ...
        if (list.length > 0) {
          ...
          setDeviceName(list[0].name ?? 'Kandang Utama');
        } else {
          setDeviceName('Belum ada device terdaftar');
        }
      })
      .catch(() => setDeviceName('Gagal memuat device'));
  }, []);
        ...
        <View style={styles.header}>
          <View style={{ flex: 1 }}>
            ...
            <View style={styles.onlineRow}>
              ...
              {lastUpdated && (
                <Text style={styles.updateText}>  Update: {lastUpdated}</Text>
              )}
            </View>
          </View>
          <TouchableOpacity
        ...
        <View style={styles.body}>
          ...
          {!deviceId && (
            <View style={styles.noDeviceBanner}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.noDeviceTitle}>Belum ada device terdaftar</Text>
                <Text style={styles.noDeviceSub}>
                  Daftarkan device via Postman: POST /api/devices, lalu buka ulang app
                </Text>
              </View>
            </View>
          )}
          ...
          <View style={styles.sensorRow}>
            <SensorCard
              icon="🌡️" label="Suhu" value={tempVal}
              status={getTempStatus(data?.temperature)}
            />
            <SensorCard
              icon="🌾" label="Level Pakan" value={feedVal}
              status={getFeedStatus(data?.feedLevel)} active
            />
            <SensorCard
              icon="☀️" label="Cahaya" value={lightVal}
              status="normal"
            />
          </View>
          ...
          <View style={styles.feedCard}>
            ...
            <View style={styles.progressBg}>
              <View style={[styles.progressFill, {
                ...
                backgroundColor:
                  getFeedStatus(data?.feedLevel) === 'danger'  ? Colors.danger
                : getFeedStatus(data?.feedLevel) === 'warning' ? Colors.warning
                : Colors.primary,
              }]} />
            </View>
          ...
          {data?.phLevel !== undefined && (
            <View style={[styles.feedCard, { flexDirection: 'row', alignItems: 'center', gap: 14 }]}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={[styles.feedPercent, { fontSize: 26,
                  color: getPhStatus(data.phLevel) === 'normal' ? Colors.primary : Colors.danger }]}>
                  {data.phLevel}
                </Text>
              </View>
              <View style={[styles.phBadge, {
                backgroundColor: getPhStatus(data.phLevel) === 'normal'
                  ? Colors.successBg : Colors.dangerBg,
              }]}>
                <Text style={[styles.phBadgeText, {
                  color: getPhStatus(data.phLevel) === 'normal'
...
const styles = StyleSheet.create({
  ...
  noDeviceBanner: { flexDirection: 'row', alignItems: 'flex-start', gap: 12, backgroundColor: Colors.infoBg, borderRadius: 14, padding: 14, borderWidth: 1, borderColor: Colors.info },
  noDeviceTitle:  { fontSize: 14, fontWeight: '700', color: Colors.info, marginBottom: 4 },
  noDeviceSub:    { fontSize: 12, color: Colors.textSecondary, lineHeight: 18 },
  ...
  btnFeedNow:     { flex: 1.4, backgroundColor: Colors.primary, borderRadius: 14, paddingVertical: 16, alignItems: 'center', shadowColor: Colors.primary, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 4 },
