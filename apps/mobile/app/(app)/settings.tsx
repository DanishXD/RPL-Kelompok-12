...
function SettingRow({ icon, title, subtitle, onPress, right }: {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.deviceStatusCard}>
          ...
          {activeDeviceId && (
            <Text style={styles.deviceId} numberOfLines={1}>ID: {activeDeviceId}</Text>
          )}
        </View>
        ...
        <View style={styles.group}>
          <SettingRow icon="👤" title="Profil Pengguna"          subtitle="Nama, nomor kontak, preferensi"  onPress={() => Alert.alert('Info', 'Fitur segera hadir')} />
          <View style={styles.divider} />
          <SettingRow icon="🔔" title="Preferensi Notifikasi"    subtitle="Atur jenis notifikasi yang diterima" onPress={() => Alert.alert('Info', 'Fitur segera hadir')} />
        </View>

        {/* Perangkat */}
        <Text style={styles.groupLabel}>Perangkat</Text>
        <View style={styles.group}>
          <SettingRow icon="📡" title="Konfigurasi WiFi"         subtitle="EcoNet-2.4G · Terhubung"         onPress={() => Alert.alert('Info', 'Fitur segera hadir')} />
          <View style={styles.divider} />
          <SettingRow icon="🖥️" title="Nama Perangkat"           subtitle="Feeder-Kandang-01"                onPress={() => Alert.alert('Info', 'Fitur segera hadir')} />
          <View style={styles.divider} />
          <SettingRow icon="🕐" title="Sinkronisasi Waktu"       subtitle="Terakhir sync: hari ini"          onPress={() => Alert.alert('Sync', 'Sinkronisasi waktu berhasil!')} />
        </View>
        ...
        <View style={styles.group}>
          ...
          />
          <View style={styles.divider} />
          <SettingRow icon="🌐" title="Bahasa" subtitle="Indonesia" onPress={() => {}} />
        </View>
        ...
        <View style={styles.group}>
          <View style={styles.helpRow}>
            <TouchableOpacity style={styles.helpBtn} activeOpacity={0.7}
              onPress={() => Alert.alert('Manual', 'Dokumentasi akan dibuka')} >
              <Text style={styles.helpIcon}>📖</Text>
              <Text style={styles.helpText}>Manual & FAQ</Text>
            </TouchableOpacity>
            <View style={styles.helpDivider} />
            <TouchableOpacity style={styles.helpBtn} activeOpacity={0.7}
              onPress={() => Alert.alert('Support', 'Hubungi: support@ecosmart.id')} >
              <Text style={styles.helpIcon}>📞</Text>
        ...
        <Text style={styles.version}>EcoSmart Feeder v1.0.0  •  Step 6</Text>
...
const styles = StyleSheet.create({
  ...
  deviceStatusCard: { backgroundColor: Colors.primaryBg, borderRadius: 14, padding: 14, gap: 6, borderWidth: 1, borderColor: Colors.primary + '40' },
