...
function ScheduleModal({ visible, onClose, onSave, editData }: ScheduleModalProps) {
  ...
  useEffect(() => {
    if (editData) {
      ...
      setTimeHour('07'); setTimeMinute('00');
      setAmount('100g'); setCustomAmt('');
      setUseCustom(false); setDays(['everyday']); setNote('');
    }
  }, [editData, visible]);
  ...
  const handleSave = async () => {
    ...
    if (isNaN(h) || h < 0 || h > 23) {
      Alert.alert('Error', 'Jam harus antara 00–23'); return;
    }
    if (isNaN(m) || m < 0 || m > 59) {
      Alert.alert('Error', 'Menit harus antara 00–59'); return;
    }
    const finalAmount = useCustom ? customAmt.trim() : amount;
    if (!finalAmount) {
      Alert.alert('Error', 'Jumlah pakan wajib diisi'); return;
    }
    if (days.length === 0) {
      Alert.alert('Error', 'Pilih minimal 1 hari'); return;
    }
    ...
    try {
      ...
      Alert.alert('Gagal', err?.response?.data?.error ?? 'Gagal menyimpan jadwal');
    } finally {
      setSaving(false);
    }
  };
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <SafeAreaView style={modal.container}>
        ...
        <View style={modal.header}>
          ...
          <Text style={modal.headerTitle}>{editData ? 'Edit Jadwal' : 'Tambah Jadwal'}</Text>
        ...
        <ScrollView contentContainerStyle={modal.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
            ...
            <View style={modal.timeRow}>
              <View style={modal.timeGroup}>
                ...
                <View style={modal.timeInputWrap}>
                  <TouchableOpacity onPress={() => setTimeHour(h => String(Math.max(0, parseInt(h)-1)).padStart(2,'0'))} style={modal.timeArrow}>
                    <Text style={modal.timeArrowText}>▲</Text>
                  </TouchableOpacity>
                  <TextInput
                    style={modal.timeInput}
                    value={timeHour}
                    onChangeText={v => setTimeHour(v.replace(/\D/,'').slice(0,2))}
                    keyboardType="number-pad"
                    maxLength={2}
                  />
                  <TouchableOpacity onPress={() => setTimeHour(h => String(Math.min(23, parseInt(h)+1)).padStart(2,'0'))} style={modal.timeArrow}>
                    <Text style={modal.timeArrowText}>▼</Text>
                  </TouchableOpacity>
                </View>
              </View>
              ...
              <View style={modal.timeGroup}>
                ...
                <View style={modal.timeInputWrap}>
                  <TouchableOpacity onPress={() => setTimeMinute(m => String(Math.max(0, parseInt(m)-1)).padStart(2,'0'))} style={modal.timeArrow}>
                    <Text style={modal.timeArrowText}>▲</Text>
                  </TouchableOpacity>
                  <TextInput
                    style={modal.timeInput}
                    value={timeMinute}
                    onChangeText={v => setTimeMinute(v.replace(/\D/,'').slice(0,2))}
                    keyboardType="number-pad"
                    maxLength={2}
                  />
                  <TouchableOpacity onPress={() => setTimeMinute(m => String(Math.min(59, parseInt(m)+1)).padStart(2,'0'))} style={modal.timeArrow}>
                    <Text style={modal.timeArrowText}>▼</Text>
                  </TouchableOpacity>
                </View>
              </View>
          ...
          <View style={modal.card}>
            <View style={modal.amountGrid}>
              {AMOUNT_PRESETS.map(a => (
                <TouchableOpacity
                  key={a}
                  style={[modal.amountChip, !useCustom && amount === a && modal.amountChipActive]}
                  onPress={() => { setAmount(a); setUseCustom(false); }}
                  activeOpacity={0.7}
                >
                  <Text style={[modal.amountChipText, !useCustom && amount === a && modal.amountChipTextActive]}>
                    {a}
                  </Text>
                </TouchableOpacity>
              ))}
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <Text style={[modal.amountChipText, useCustom && modal.amountChipTextActive]}>Custom</Text>
              </TouchableOpacity>
            </View>
            {useCustom && (
          ...
          <View style={modal.card}>
            <View style={modal.daysGrid}>
              {DAY_OPTIONS.map(opt => {
                ...
                return (
                  <TouchableOpacity
                    key={opt.key}
                    style={[modal.dayChip, isSelected && modal.dayChipActive, opt.key === 'everyday' && modal.dayChipEveryday]}
                    onPress={() => toggleDay(opt.key)}
                    activeOpacity={0.7}
                  >
                    <Text style={[modal.dayChipText, isSelected && modal.dayChipTextActive]}>
                      {opt.short}
                    </Text>
                  </TouchableOpacity>
                );
            ...
            <Text style={modal.daysSelected}>
              Terpilih: {formatDays(days)}
            </Text>
          </View>
...
export default function ControlScreen() {
  ...
  const executeFeed = () => {
    ...
    const interval = setInterval(() => {
      setCountdown(c => {
        if (c <= 1) {
          clearInterval(interval); setFeeding(false);
  ...
  const handleFeedNow = () => {
    if (data?.feedLevel !== undefined && data.feedLevel < 5) {
      Alert.alert('⚠️ Pakan Hampir Habis', `Level pakan hanya ${data.feedLevel}%. Lanjutkan?`, [
        { text: 'Batal', style: 'cancel' },
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.card}>
          <View style={styles.manualRow}>
            ...
            <View style={[styles.readyBadge, { borderColor: !feeding&&relayOn?Colors.primary:Colors.textMuted, backgroundColor: !feeding&&relayOn?Colors.primaryBg:Colors.borderLight }]}>
              <Text style={[styles.readyText, { color: !feeding&&relayOn?Colors.primary:Colors.textMuted }]}>
                {!feeding&&relayOn ? 'Relay Siap' : feeding ? `${countdown}s...` : 'Relay OFF'}
              </Text>
            </View>
          </View>
          ...
          {[
            ...
            <React.Fragment key={item.label}>
              <View style={styles.switchRow}>
                ...
                <View style={[styles.statusPill, { backgroundColor: item.val?Colors.primaryBg:Colors.borderLight }]}>
                  <Text style={[styles.statusPillText, { color: item.val?Colors.primary:Colors.textMuted }]}>{item.val?'ON':'OFF'}</Text>
                </View>
              </View>
        ...
        <View style={styles.schedulesHeader}>
          <Text style={styles.sectionTitle}>Jadwal Otomatis ({schedules.length})</Text>
          {deviceId && (
            <TouchableOpacity style={styles.refreshBtn} onPress={() => loadSchedules(deviceId)}>
        ...
        {loadingSched ? (
          ...
          <View style={styles.emptyCard}>
            ...
            <Text style={styles.emptyText}>Belum ada jadwal. Tambah jadwal pertama kamu!</Text>
          </View>
        ) : (
          <View style={styles.card}>
            {schedules.map((s, i, arr) => (
              <React.Fragment key={s.id}>
                <View style={styles.scheduleRow}>
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.scheduleTime, (override || !s.isActive) && { color: Colors.textMuted }]}>
                      {s.time}
                    </Text>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => { setEditSchedule(s); setModalVisible(true); }}
                  ...
                  <Switch
                    ...
                    thumbColor={(s.isActive&&!override)?Colors.primary:Colors.white}
                  />
                </View>
                {i < arr.length-1 && <View style={styles.divider} />}
              </React.Fragment>
        ...
        <TouchableOpacity
          style={styles.addBtn}
          onPress={() => { setEditSchedule(null); setModalVisible(true); }}
          activeOpacity={0.85}
        >
      ...
      <ScheduleModal
        ...
        onClose={() => { setModalVisible(false); setEditSchedule(null); }}
...
const styles = StyleSheet.create({
  ...
  schedulesHeader:  { flexDirection:'row', justifyContent:'space-between', alignItems:'center' },
  refreshBtn:       { width:36, height:36, borderRadius:18, backgroundColor:Colors.bgCard, alignItems:'center', justifyContent:'center', borderWidth:1, borderColor:Colors.border },
  ...
  editBtn:       { width:32, height:32, borderRadius:16, backgroundColor:Colors.primaryBg, alignItems:'center', justifyContent:'center' },
  editBtnText:   { fontSize:14 },
  deleteBtn:     { width:32, height:32, borderRadius:16, backgroundColor:Colors.dangerBg, alignItems:'center', justifyContent:'center' },
  deleteBtnText: { fontSize:14 },

  addBtn:     { backgroundColor:Colors.primary, borderRadius:14, paddingVertical:16, flexDirection:'row', alignItems:'center', justifyContent:'center', gap:10, elevation:4 },
...
const modal = StyleSheet.create({
  ...
  saveBtn:   { backgroundColor:'rgba(255,255,255,0.2)', paddingHorizontal:14, paddingVertical:6, borderRadius:20, minWidth:64, alignItems:'center' },
  saveText:  { fontSize:15, color:Colors.white, fontWeight:'700' },

  scroll:        { padding:16, gap:14, paddingBottom:40 },
  sectionLabel:  { fontSize:11, fontWeight:'700', color:Colors.textMuted, letterSpacing:0.8, marginBottom:-6 },
  ...
  timeArrow:     { width:44, height:32, backgroundColor:Colors.bgPage, borderRadius:8, alignItems:'center', justifyContent:'center', borderWidth:1, borderColor:Colors.border },
  timeArrowText: { fontSize:12, color:Colors.primary, fontWeight:'700' },
  timeInput:     { width:64, height:56, backgroundColor:Colors.bgPage, borderRadius:12, textAlign:'center', fontSize:28, fontWeight:'800', color:Colors.primary, borderWidth:2, borderColor:Colors.primary },
  timeSeparator: { fontSize:32, fontWeight:'800', color:Colors.primary, marginBottom:20, paddingHorizontal:4 },
  timePreview:   { flex:1, alignItems:'center', backgroundColor:Colors.primaryBg, borderRadius:12, padding:12 },
  timePreviewLabel: { fontSize:11, color:Colors.textMuted, fontWeight:'600', letterSpacing:0.5 },
  timePreviewValue: { fontSize:28, fontWeight:'800', color:Colors.primary, marginTop:4 },

  amountGrid:    { flexDirection:'row', flexWrap:'wrap', gap:10 },
  amountChip:    { paddingHorizontal:16, paddingVertical:10, borderRadius:20, backgroundColor:Colors.bgPage, borderWidth:1.5, borderColor:Colors.border },
  amountChipActive: { backgroundColor:Colors.primaryBg, borderColor:Colors.primary },
  amountChipText:    { fontSize:14, fontWeight:'600', color:Colors.textSecondary },
  amountChipTextActive: { color:Colors.primary },
  customAmtRow:  { marginTop:10 },
  customAmtInput:{ backgroundColor:Colors.bgPage, borderRadius:12, padding:14, fontSize:16, color:Colors.textPrimary, borderWidth:1.5, borderColor:Colors.primary },

  daysGrid:      { flexDirection:'row', flexWrap:'wrap', gap:8 },
  dayChip:       { paddingHorizontal:14, paddingVertical:8, borderRadius:20, backgroundColor:Colors.bgPage, borderWidth:1.5, borderColor:Colors.border },
  dayChipActive: { backgroundColor:Colors.primaryBg, borderColor:Colors.primary },
  ...
  daysSelected:  { fontSize:12, color:Colors.textMuted, marginTop:10, fontStyle:'italic' },

  noteInput:     { minHeight:80, backgroundColor:Colors.bgPage, borderRadius:12, padding:14, fontSize:14, color:Colors.textPrimary, borderWidth:1, borderColor:Colors.border, textAlignVertical:'top' },
});
