    ...
    <TouchableOpacity style={[styles.card, active && styles.cardActive]} onPress={onPress} activeOpacity={0.8}>
      ...
      {statusLabel && (
        <View style={[styles.badge, { backgroundColor: statusColors[status].bg }]}>
          <Text style={[styles.badgeText, { color: statusColors[status].text }]}>{statusLabel}</Text>
        </View>
      )}
    </TouchableOpacity>
  );
...
const styles = StyleSheet.create({
  card:        { flex: 1, backgroundColor: Colors.bgCard, borderRadius: 16, padding: 14, alignItems: 'center', gap: 4, borderWidth: 1.5, borderColor: Colors.border, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2 },
