  ...
  return (
    <View style={styles.wrap}>
      ...
      <View style={[styles.inputWrap, focused && styles.focused, !!error && styles.errBorder]}>
        <TextInput style={styles.input} placeholderTextColor={Colors.textMuted} secureTextEntry={isPassword && !show} autoCapitalize="none" autoCorrect={false} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)} {...props} />
        {isPassword && <TouchableOpacity onPress={() => setShow(v=>!v)} style={styles.eye}><Ionicons name={show?'eye-off-outline':'eye-outline'} size={20} color={Colors.textSecondary} /></TouchableOpacity>}
      </View>
      {!!error && <View style={styles.errRow}><Ionicons name="alert-circle-outline" size={13} color={Colors.danger} /><Text style={styles.errText}>{error}</Text></View>}
    </View>
  );
}
